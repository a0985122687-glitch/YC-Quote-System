...
def check_password():
    def password_entered():
        if st.session_state["password"] == "yc888":  # 👈 這是你的密碼，以後想改來這裡改
    ...
    if "password_correct" not in st.session_state:
        st.title("🛡️ 御晁工程系統授權驗證")
        st.text_input("請輸入專屬授權碼以開啟系統", type="password", on_change=password_entered, key="password")
        return False
    elif not st.session_state["password_correct"]:
        st.title("🛡️ 御晁工程系統授權驗證")
        st.text_input("授權碼不正確，請重新輸入", type="password", on_change=password_entered, key="password")
        st.error("🔒 授權失敗，請聯繫管理員。")
        return False
    return True
